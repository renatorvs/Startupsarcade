...
--
-- Host: 127.0.0.1:3306
-- Tempo de geração: 20-Set-2022 às 01:06
...
--
-- Estrutura stand-in para vista `blog_categoria_post`
-- (Veja abaixo para a view atual)
--
DROP VIEW IF EXISTS `blog_categoria_post`;
CREATE TABLE IF NOT EXISTS `blog_categoria_post` (
`blogcat_id` int(11)
...
(13, 'titulo blog DESTAQUE', '::Subtitulo_artigo:;', 'post description:', NULL, '', 1, 1, 1, 1, 1),
(14, 'Política de Privacidade do startups arcade', 'Informações legais do startups arcade:', 'Se você não reside na Região Europeia, o startups arcade fornece nossos Serviços para você de acordo com estes Termos de Serviço e esta Política de Privacidade.', NULL, '', 1, 1, 1, 1, 1),
(15, 'aaaaaaaaaaaaa', 'aaassssssss', 'sssssssssssssssss', '1663035931', 'ssssssssssss', 1, 1, 1, 1, 1);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `blog_post_categoria`  AS  select `b`.`blog_id` AS `blog_id`,`b`.`blogdestaque_id` AS `blogdestaque_id`,`b`.`blog_subtitulo` AS `blog_subtitulo`,`b`.`blog_text` AS `blog_text`,`b`.`blog_date` AS `blog_date`,`b`.`blog_img` AS `blog_img`,`b`.`blog_img_alt` AS `blog_img_alt`,`b`.`blog_video_src` AS `blog_video_src`,`b`.`blog_autor` AS `blog_autor`,`b`.`blogpost_id` AS `blogpost_id`,`b`.`blog_fonte` AS `blog_fonte`,`p`.`post_id` AS `post_id`,`p`.`post_titulo` AS `post_titulo`,`p`.`post_subtitulo` AS `post_subtitulo`,`p`.`post_description` AS `post_description`,`p`.`post_img` AS `post_img`,`p`.`post_img_alt` AS `post_img_alt`,`p`.`postcat_id` AS `postcat_id`,`p`.`tipo_post_id` AS `tipo_post_id`,`p`.`userpost_id` AS `userpost_id`,`p`.`post_paisid` AS `post_paisid`,`cat`.`blogcat_id` AS `blogcat_id`,`cat`.`blogcat_nome` AS `blogcat_nome`,`cat`.`blogpais_id` AS `blogpais_id`,`bd`.`blogdest_id` AS `blogdest_id`,`bd`.`blogdest_nome` AS `blogdest_nome` from (((`blog` `b` join `post` `p` on(`b`.`blogpost_id` = `p`.`post_id`)) join `blog_categoria` `cat` on(`p`.`postcat_id` = `cat`.`blogcat_id`)) join `blog_destaque` `bd` on(`b`.`blogdestaque_id` = `bd`.`blogdest_id`)) ;
